...
public static class Perlin
{
    private static readonly int[] perm = {
        ...
        236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180, 151
    };

    public static float Noise(float x, float y)
    {
        int xCoord = Mathf.FloorToInt(x) & 0xff;
        int yCoord = Mathf.FloorToInt(y) & 0xff;
        x -= Mathf.Floor(x);
        y -= Mathf.Floor(y);
        float u = Fade(x);
        float v = Fade(y);
        int a = (perm[xCoord] + yCoord) & 0xff;
        int b = (perm[xCoord + 1] + yCoord) & 0xff;
